...
var importFromText = async (text, path) => {
  ...
  globalThis.REMOTEESM_TEXT_REFERENCES[path] = uri;
  return imported;
};
var resolve = get;
...
// utils.ts
var isSrc = (str) => {
  return str && typeof str === "string" && Object.values(languages_exports).find((arr) => arr.includes(str.split(".").slice(-1)[0]));
};
...
var importFromText2 = async (text, path) => {
  const extension = path.split(".").slice(-1)[0];
  const isJSON = extension === "json";
  let mimeType = isJSON ? "application/json" : "application/javascript";
...
var DOMService = class extends Service {
  constructor(options, parentNode, interpreters) {
    ...
    this.components = {};
    this.templates = {};
    this.resolveNode = (element, options2) => {
      let node;
      if (this.nodes.get(options2.id)?.element?.parentNode?.id === options2.parentNode || this.nodes.get(options2.id)?.parentNode === options2.parentNode) {
        ...
        node = new GraphNode(options2, options2.parentNode ? this.nodes.get(options2.parentNode) : this.parentNode, this);
      }
    ...
    this.addElement = (options2, generateChildElementNodes = false) => {
      ...
      let node = this.resolveNode(elm, options2);
      ...
      this.resolveParentNode(elm, options2, oncreate);
    ...
    this.updateOptions = (options2, element) => {
      ...
      if (typeof options2.parentNode === "string" && document.getElementById(options2.parentNode))
        options2.parentNode = document.getElementById(options2.parentNode);
      if (!options2.parentNode) {
        if (!this.parentNode)
          this.parentNode = document.body;
    ...
    this.addComponent = (options2, generateChildElementNodes = true) => {
      ...
      this.resolveParentNode(elm, options2);
...
var ESPlugin = class {
  ...
  constructor(node, options = {}) {
    ...
    console.log('Creating plugin', this.#options.tag, node)
  ...
  #activate = () => {
    if (this.initial.graph) {
      ...
      if (ports) {
        ...
        const gotOutput = this.graph.get(output);
        gotOutput.subscribe((...args) => {
          for (let tag in this.graph.children)
            this.#runGraph(this.graph.children[tag], ...args);
        });
      }
      for (let tag in graph.nodes) {
...
var _filesystem, _input, _options, _url, _cache, _plugin, _onImport, _throw;
var WASL = class {
  constructor(urlOrObject, options = {}, url) {
    ...
    this.load = async (node, info, options, id) => {
      if (node.plugins) {
        for (let nestedName in node.plugins) {
          const nestedNode = node.graph?.nodes?.[nestedName];
          if (node.plugins) {
    ...
    this.resolve = async (target, info, options, graph = {}) => {
      ...
      relativeToResolved = options._remote || relativeToResolved || mainPath;
      for (let name2 in target) {
        const node = target[name2];
        const isObj = node && typeof node === "object" && !Array.isArray(node);
        if (isObj) {
          await this.load(node, info, options, id);
          let ogSrc = node.src ?? "";
          if (isSrc(ogSrc) || node.graph) {
            node.src = null;
            let passToNested = void 0;
            ...
            try {
              ...
              fullPath = relativeToResolved = relativeToResolved ? resolve2(ogSrc, mainPath) : resolve2(ogSrc);
            }
            if (ogSrc){
            if (isImportMode) {
              node.src = await this.get(fullPath, void 0);
              if (_remote) {
                if (!node.src) {
                  const got = await this.resolve([node], info, options, { nodes: [node] });
                  const val = got[0];
                  node.src = "src" in val ? val.src : val;
                  passToNested = resolve2(ogSrc);
                  console.log('IS REMOTe',passToNested )
                }
              }
              passToNested = resolve2(ogSrc, url, true);
              console.log('IS NOT REMOT',passToNested )
            ...
            if (node.src?.graph)
              node.graph = node.src.graph;
        } else console.warn('NOT IG SRC', fullPath)
            if (node.graph) {
                console.log('node.graph', fullPath, passToNested, JSON.parse(JSON.stringify(node)))
              node.src = await this.init(passToNested ?? node, {
    ...
    this.init = async (urlOrObject = __privateGet(this, _input), options = __privateGet(this, _options), url = __privateGet(this, _url) ?? "") => {
      ...
      console.log('INPUTS', urlOrObject, options,relativeTo, url)

      if (!internalLoadCall) {
        __privateSet(this, _filesystem, options.filesystem);
      }
      const isString = typeof urlOrObject === "string";
      let object, relativeToResolved = !internalLoadCall || url ? "" : relativeTo;
      if (url || isString) {
        if (!url)
          url = urlOrObject;
      ...
      console.log('GOT OBJ', object, url, urlOrObject)
      ...
      if (this.errors.length === 0) {
        ...
        console.log('RESILVING NODES', nodes)
        ...
        if (!internalLoadCall) {
          if (clonedOptions.output !== "object") {
            __privateSet(this, _plugin, new src_default(object, {
    ...
    this.start = async () => {
      if (__privateGet(this, _plugin))
        return await __privateGet(this, _plugin).start();
    };
    this.stop = async () => {
      if (__privateGet(this, _plugin))
        return await __privateGet(this, _plugin).stop();
    };
    __privateSet(this, _input, urlOrObject);
    __privateSet(this, _options, options);
